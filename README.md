...
- Experimental PWM-safe software dimming on Windows through the display gamma ramp, so the dark profile can dim pixels without lowering monitor hardware brightness.
...
- PWM-safe software dimming is off by default and Windows-only. It stores the current gamma ramps before dimming, restores them before each mode change, and restores them again when the app quits or the Python process exits. Because f.lux, HDR, Night light, graphics drivers, and calibration tools can also own the gamma/LUT pipeline, the app reapplies software dimming after f.lux changes and may show a warning if Windows or the driver rejects gamma changes.
